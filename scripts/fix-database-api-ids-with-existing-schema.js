  ...
  try {
    ...
    if (allLeagues && allLeagues.length > 0) {
      allLeagues.forEach((league, i) => {
        console.log(`${(i+1).toString().padStart(2)} - ${league.name} | API Data: ${league.api_data ? 'EXISTS' : 'NULL'}`);
      });
    } else {
    ...
    console.log('\n🔧 UPDATING API DATA WITH CORRECT IDs...');
    let updated = 0;
    ...
    for (const league of allLeagues) {
      const correctApiId = leagueApiMappings[league.name];
      
      if (correctApiId) {
        // Check if api_data already has the correct ID
        let currentApiData = {};
        try {
        ...
        if (currentApiData.highlightly_id !== correctApiId) {
          // Update the api_data with the correct Highlightly ID
          ...
          if (!error) {
            console.log(`✅ Updated: ${league.name} -> Highlightly ID: ${correctApiId}`);
            updated++;
          } else {
        ...
        console.log(`⚠️ No API mapping found for: ${league.name}`);
        noMapping++;
      }
    }
    ...
    console.log(`⚠️ No mapping: ${noMapping} leagues`);
    
    // Show final result - leagues with API data
    ...
    console.log('\n📋 LEAGUES WITH API DATA (sample):');
    finalSample?.forEach(l => {
      try {
        const apiData = JSON.parse(l.api_data);
        console.log(`- ${l.name}: Highlightly ID ${apiData.highlightly_id || 'N/A'}`);
      } catch (e) {
        console.log(`- ${l.name}: Invalid API data`);
      }
    });
    
    const { count: totalWithApiData } = await supabase
      .from('leagues')
      .select('*', { count: 'exact', head: true })
      .not('api_data', 'is', null);
    
    console.log(`\n📈 Total leagues with API data: ${totalWithApiData}`);
