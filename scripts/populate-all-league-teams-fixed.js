...
async function populateAllLeagueTeamsFixed() {
  console.log('🏗️ POPULATING ALL TEAMS FOR ALL LEAGUES (FIXED VERSION)...');
  ...
  try {
    ...
    for (const league of leagues) {
      try {
        ...
        // Parse the JSON string in api_data
        let apiData;
        try {
          apiData = typeof league.api_data === 'string' ? JSON.parse(league.api_data) : league.api_data;
        } catch (parseError) {
          console.log(`  ❌ Failed to parse API data for ${league.name}: ${parseError.message}`);
          failedLeagues++;
          continue;
        }
        ...
        try {
          ...
          if (teamsData?.data && Array.isArray(teamsData.data) && teamsData.data.length > 0) {
            console.log(`  ✅ Found ${teamsData.data.length} teams from teams endpoint`);
            ...
            console.log(`  🔄 No teams data, trying standings...`);
            ...
            if (standingsData?.data && Array.isArray(standingsData.data)) {
              ...
              if (teamsFromStandings > 0) {
                console.log(`  ✅ Found ${teamsFromStandings} teams from standings`);
                successfulLeagues++;
              } else {
                // Try matches endpoint as last resort
                console.log(`  🔄 No standings data, trying recent matches...`);
                
                const matchesData = await callHighlightlyAPI('matches', {
                  leagueId: highlightlyId,
                  season: '2025',
                  date: '2025-01-01' // Get matches from this season
                });
                
                if (matchesData?.data && Array.isArray(matchesData.data)) {
                  let teamsFromMatches = 0;
                  ...
                  if (teamsFromMatches > 0) {
                    console.log(`  ✅ Found ${teamsFromMatches} teams from matches`);
                    successfulLeagues++;
                  } else {
                  ...
                  console.log(`  ❌ No data available for ${league.name}`);
                  failedLeagues++;
                }
              }
            } else {
...
populateAllLeagueTeamsFixed().catch(console.error); 