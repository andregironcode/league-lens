/**
 * ULTIMATE EXPANSION TO 300+ MATCHES
 * 
 * Comprehensive script to get all major leagues to 300+ matches
 * by trying multiple seasons (2024, 2023, 2022, 2021, 2020) and different approaches
 */
...
// Extended league configurations with more seasons to try
const LEAGUE_CONFIGS = [
  { id: 33973, name: 'Premier League', country: 'England', seasons: [2024, 2023, 2022, 2021, 2020, 2019] },
  { id: 119924, name: 'La Liga', country: 'Spain', seasons: [2024, 2023, 2022, 2021, 2020, 2019] },
  { id: 115669, name: 'Serie A', country: 'Italy', seasons: [2024, 2023, 2022, 2021, 2020, 2019] },
  { id: 67162, name: 'Bundesliga', country: 'Germany', seasons: [2024, 2023, 2022, 2021, 2020, 2019] },
  { id: 52695, name: 'Ligue 1', country: 'France', seasons: [2024, 2023, 2022, 2021, 2020, 2019] },
  { id: 2486, name: 'UEFA Champions League', country: 'World', seasons: [2024, 2023, 2022, 2021, 2020, 2019] },
  { id: 3337, name: 'UEFA Europa League', country: 'World', seasons: [2024, 2023, 2022, 2021, 2020, 2019] }
];
...
async function expandLeagueUltimate(leagueConfig) {
  const { id, name, country, seasons } = leagueConfig;
  
  console.log(`\n⚽ ULTIMATE EXPANSION: ${name} [${country}]`);
  ...
  // Try each season with multiple attempts
  for (const season of seasons) {
    if (currentTotal >= TARGET_MATCHES) break;
    
    console.log(`\n🗓️  Trying season ${season}...`);
    
    // Try multiple approaches for each season
    const approaches = [
      { endpoint: '/matches', params: { leagueId: parseInt(id), season: parseInt(season) } },
      { endpoint: '/matches', params: { leagueId: parseInt(id), season: parseInt(season), limit: 500 } },
      { endpoint: '/matches', params: { leagueId: parseInt(id), season: parseInt(season), status: 'finished' } },
      { endpoint: '/matches', params: { leagueId: parseInt(id), season: parseInt(season), status: 'scheduled' } }
    ];
    
    let seasonAdded = 0;
    let bestMatches = [];
    
    // Try each approach and use the one with most matches
    for (const approach of approaches) {
      const matches = await makeAPICall(approach.endpoint, approach.params);
      if (matches && matches.length > bestMatches.length) {
        bestMatches = matches;
      }
    ...
    console.log(`     📡 Found ${bestMatches.length} matches from season ${season}`);
    
    // Process matches in batches
    const batchSize = 50;
    for (let i = 0; i < bestMatches.length && currentTotal < TARGET_MATCHES; i += batchSize) {
      ...
      // Progress update
      if (i + batchSize < bestMatches.length && currentTotal < TARGET_MATCHES) {
        console.log(`     📈 Progress: ${seasonAdded} added so far from season ${season}`);
      }
    }
  ...
  console.log(`\n${success ? '✅' : '⚠️'} ${name} ULTIMATE EXPANSION COMPLETE:`);
...
async function ultimateExpansionToTarget() {
  console.log('🚀 ULTIMATE EXPANSION TO 300+ MATCHES');
  console.log('=' .repeat(80));
  console.log(`🎯 Target: ${TARGET_MATCHES} matches per league`);
  console.log(`📅 Seasons: 2024, 2023, 2022, 2021, 2020, 2019`);
  console.log(`⚽ Leagues: ${LEAGUE_CONFIGS.length} major leagues`);
  console.log(`🔄 Multiple API approaches per season`);
  ...
  for (let i = 0; i < LEAGUE_CONFIGS.length; i++) {
    ...
    console.log(`\n[${i + 1}/${LEAGUE_CONFIGS.length}] ${'='.repeat(60)}`);
    
    const result = await expandLeagueUltimate(league);
    results.push({
      league: league.name,
  ...
  if (stillNeedExpansion.length > 0) {
    ...
    console.log(`\n💡 Next steps:`);
    console.log(`   1. Try more seasons (2018, 2017, 2016)`);
    console.log(`   2. Try different API endpoints`);
    console.log(`   3. Consider adding more teams to enable more matches`);
  } else {
    console.log(`\n🎉 ALL MAJOR LEAGUES NOW HAVE 300+ MATCHES!`);
  }
}

ultimateExpansionToTarget(); 