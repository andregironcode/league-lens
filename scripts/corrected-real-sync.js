...
async function clearExistingData() {
  ...
  try {
    ...
    const { error: highlightsError } = await supabase.from('highlights').delete().neq('id', '0');
    if (highlightsError) console.log('Highlights clear error:', highlightsError.message);
    
    const { error: matchesError } = await supabase.from('matches').delete().neq('id', '0');
    if (matchesError) console.log('Matches clear error:', matchesError.message);
    
    const { error: teamsError } = await supabase.from('teams').delete().neq('id', '0');
    if (teamsError) console.log('Teams clear error:', teamsError.message);
    
    const { error: leaguesError } = await supabase.from('leagues').delete().neq('id', '0');
    if (leaguesError) console.log('Leagues clear error:', leaguesError.message);
...
// Sync leagues with CORRECT database structure
      ...
      if (response && Array.isArray(response) && response[0]) {
        ...
        const { data, error } = await supabase
          ...
          .upsert({
            ...
            country_name: leagueData.country?.name || league.country,
        ...
        if (error) {
          console.log(`❌ Error syncing league ${leagueData.name}:`, error.message);
        } else {
...
// Sync teams from standings with CORRECT database structure
      ...
      if (data?.groups?.[0]?.standings) {
        ...
        for (const standing of standings) {
          ...
          if (team) {
            ...
            if (error) {
              console.log(`❌ Error syncing team ${team.name}:`, error.message);
            } else {
...
// Sync matches with CORRECT database structure
    ...
    for (const league of PRIORITY_LEAGUES) {
      ...
      if (data?.data) {
        for (const match of data.data) {
          ...
          const matchTime = matchDateTime.toTimeString().split(' ')[0];
          ...
          if (error) {
            console.log(`❌ Error syncing match ${match.homeTeam?.name} vs ${match.awayTeam?.name}:`, error.message);
          } else {
...
// Sync highlights with CORRECT database structure
        ...
        for (const highlight of data.data) {
          const { data: insertData, error } = await supabase
            ...
            .upsert({
              ...
              match_id: highlight.match?.id?.toString(),
          ...
          if (error) {
            console.log(`❌ Error syncing highlight ${highlight.title}:`, error.message);
          } else {
...
// Update sync status
async function updateSyncStatus(tableName, status, recordsCount = 0) {
  ...
  if (error) {
    console.log(`❌ Error updating sync status for ${tableName}:`, error.message);
  }
}

// Main sync function
async function runCorrectedRealSync() {
  console.log('🚀 Starting CORRECTED Real Data Synchronization');
  console.log('==================================================');
  console.log('🔧 Using CORRECT database structure');
  console.log('📊 Will populate with REAL teams, matches, and highlights');
  console.log('🎯 Fixed column mapping to match actual database!');
  ...
  try {
    ...
    await syncLeagues();
    await updateSyncStatus('leagues', 'completed', PRIORITY_LEAGUES.length);
    ...
    console.log('\n🎉 CORRECTED REAL DATA SYNC COMPLETED SUCCESSFULLY!');
    console.log('==================================================');
    ...
    console.log('🔧 All data properly saved to database with correct structure!');
