...
export const fetchHighlightsFromApi = async (matchId?: string): Promise<any> => {
  try {
    const queryParams = matchId ? `?matchId=${matchId}` : '';
    
    const { data, error } = await supabase.functions.invoke('get-highlights' + queryParams);
    ...
    console.error('Error in fetchHighlightsFromApi:', error);
    throw error;
  }
};

// This function will be updated later to use the real API once integration is working
export const getRecommendedHighlights = async (): Promise<MatchHighlight[]> => {
  // First try to fetch from the API
  try {
    const highlightsData = await fetchHighlightsFromApi();
    console.log('API response:', highlightsData);
    
    // Here you would transform the API response to match your MatchHighlight type
    // This is a placeholder for now - we'll need to adapt this based on the actual API response
    ...
    // Return the existing mock data (keep the current implementation)
    return [
      {
        id: '1',
...
// Keep the other existing functions but update the getLeagueHighlights to use the API
export const getLeagueHighlights = async (): Promise<League[]> => {
  // First try to fetch from the API
  try {
    const highlightsData = await fetchHighlightsFromApi();
    console.log('API response for league highlights:', highlightsData);
    
    // This is a placeholder - we'll need to adapt this based on the actual API response
    // For now, fall back to mock data
    throw new Error('API response transformation not implemented yet');
  } catch (error) {
    console.warn('Failed to fetch league highlights from API, falling back to mock data:', error);
...
// Keep the remaining functions from the original file
