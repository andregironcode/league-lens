...
const LeaguePage: React.FC<LeaguePageProps> = ({ league, onBack }) => {
  ...
  const fetchAllMatches = async () => {
    try {
      ...
      console.log(`[LeaguePage] Fetching comprehensive match history for league ${league.id}`);
      
      // Try to get more matches with higher limit and different approaches
      const responses = await Promise.all([
        ...
        highlightlyClient.getMatches({
          ...
          console.log('[LeaguePage] Current matches failed:', err.message);
          return null;
        }),
        
        // Try with season parameter if available
        league.seasons?.[0] ? highlightlyClient.getMatches({
          leagueId: league.id,
          season: league.seasons[0].season.toString(),
          limit: '100'
        }).catch(err => {
          console.log('[LeaguePage] Season matches failed:', err.message);
          return null;
        }) : null,
        ...
        highlightlyClient.getMatches({
          ...
          console.log('[LeaguePage] 2025 season matches failed:', err.message);
          return null;
        })
      ]);
      
      console.log(`[LeaguePage] Match responses:`, responses.map(r => r ? (Array.isArray(r) ? r.length : 'object') : 'null'));
      ...
      console.log(`[LeaguePage] Total unique matches found: ${allMatchesData.length}`);
      
      // Sort matches by date (most recent first)
      allMatchesData.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
      
      // Separate finished and upcoming matches more intelligently
      const now = new Date();
      ...
      allMatchesData.forEach((match: Match) => {
        ...
        const hasScore = match.goals && (match.goals.home !== undefined || match.goals.away !== undefined);
        
        // More sophisticated logic for match status
        if (hasScore || matchDate < new Date(now.getTime() - 2 * 60 * 60 * 1000)) {
          finishedMatches.push(match);
        } else if (matchDate > now) {
          upcomingMatchesData.push(match);
        } else {
          // Ambiguous cases - probably finished but no score data
          finishedMatches.push(match);
        }
      });
      
      console.log(`[LeaguePage] Categorized matches - Finished: ${finishedMatches.length}, Upcoming: ${upcomingMatchesData.length}`);
      console.log(`[LeaguePage] Sample finished match scores:`, finishedMatches.slice(0, 3).map(m => ({
        date: m.date,
        score: m.goals ? `${m.goals.home}-${m.goals.away}` : 'No score',
        teams: `${m.homeTeam.name} vs ${m.awayTeam.name}`
      })));
      
      setAllMatches(allMatchesData);
      setRecentMatches(finishedMatches.slice(0, 10)); // Show more recent matches
      setUpcomingMatches(upcomingMatchesData.slice(0, 10));
      
      console.log(`[LeaguePage] Loaded comprehensive match data - Total: ${allMatchesData.length}, Recent: ${finishedMatches.length}, Upcoming: ${upcomingMatchesData.length}`);
      
    } catch (err) {
      console.error('[LeaguePage] Error fetching all matches:', err);
    } finally {
      setMatchesLoading(false);
    }
  };
    ...
    <div className="bg-[#1a1a1a] rounded-lg overflow-hidden">
      ...
      {matches.length === 0 ? (
        ...
        <div className="divide-y divide-gray-700/30">
          {matches.map((match) => (
            <div key={match.id} className="p-4 hover:bg-[#222222] transition-colors">
              <div className="flex items-center justify-between">
                ...
                <div className="ml-4">
                  <span className={`text-xs px-2 py-1 rounded ${
                    match.goals ? 'bg-green-600 text-white' :
                    new Date(match.date) > new Date() ? 'bg-blue-600 text-white' :
                    'bg-gray-600 text-white'
                  }`}>
                    {match.goals ? 'FT' : new Date(match.date) > new Date() ? 'SCH' : 'N/A'}
                  </span>
                </div>
              </div>
            </div>
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4 flex-1">
                                ...
                                <div className="flex items-center gap-2 px-4 flex-shrink-0">
                                  {match.goals && (match.goals.home !== undefined && match.goals.away !== undefined) ? (
                              ...
                              <div className="ml-4 text-right flex-shrink-0">
                                <div className={`text-xs px-2 py-1 rounded mb-1 ${
                                  match.goals && (match.goals.home !== undefined && match.goals.away !== undefined) ? 'bg-green-600 text-white' :
                                  new Date(match.date) > new Date() ? 'bg-blue-600 text-white' :
                                  'bg-gray-600 text-white'
                                }`}>
                                  {match.goals && (match.goals.home !== undefined && match.goals.away !== undefined) ? 'FT' : 
