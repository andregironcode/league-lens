...
export function getTopLeaguesWithUpcomingGames(matches, dateRange) {
  ...
  const leaguesWithGames = Object.keys(leagueMatches)
    .map(Number)
    .filter(leagueId => {
      return leagueMatches[leagueId] && leagueMatches[leagueId].some(match => {
        const matchDate = new Date(match.utc_date || match.match_date);
